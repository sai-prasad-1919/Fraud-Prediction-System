...
#### Create Fraud Case
```bash
curl -X POST http://localhost:8000/admin/cases/create \
  -H "Content-Type: application/json" \
  -d '{"user_id":"USER0001","risk_level":3,"admin_id":"Adminsai01"}'
```
...
### API Not Responding
1. Check backend is running: open Task Manager or run `Get-Process python` in PowerShell
2. Test connection: `curl http://localhost:8000/`
3. Check logs: `Get-Content backend\utils\logs\fraud_prediction.log -Tail 50` (PowerShell)

### Database Not Connecting
1. Verify PostgreSQL running: check Services in Windows or run `pg_isready -h localhost` (if pg tools in PATH)
